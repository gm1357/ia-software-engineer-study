...
server.registerTool(
  "encrypt_message",
  {
    ...
    outputSchema: {
      encryptedMessage: z
        .string()
        .describe("The encrypted message (format: iv:ciphertext)"),
    },
  },
...
server.registerTool(
  "decrypt_message",
  {
    ...
    inputSchema: {
      ...
      encryptionKey: z
        .string()
        .describe("The same passphrase used during encryption"),
    },
    outputSchema: {
    ...
    try {
      ...
      return {
        ...
        content: [
          {
            type: "text",
            text: `Failed to decrypt message! Check if the encrypted message and encryption key are correct. Error details: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
