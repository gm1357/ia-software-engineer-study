...
export class PreferencesService {
  ...
  async storeSummary(
    ...
    const data = {
      ...
      favorite_genres: summary.favoriteGenres
        ? JSON.stringify(summary.favoriteGenres)
        : null,
      favorite_bands: summary.favoriteBands
        ? JSON.stringify(summary.favoriteBands)
        : null,
  ...
  async getSummary(userId: string): Promise<ConversationSummary | null> {
    ...
    return {
      ...
      favoriteGenres: row.favorite_genres
        ? JSON.parse(row.favorite_genres)
        : undefined,
      favoriteBands: row.favorite_bands
        ? JSON.parse(row.favorite_bands)
        : undefined,
  ...
  async getBasicInfo(userId: string): Promise<string | undefined> {
    ...
    if (summary.favoriteBands?.length) {
      parts.push(
        `Artistas/Bandas Favoritas: ${summary.favoriteBands.join(", ")}`,
      );
    }
