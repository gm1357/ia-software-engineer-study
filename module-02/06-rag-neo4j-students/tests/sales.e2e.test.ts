...
describe("Sales Analytics Reporter - E2E Tests", async () => {
  ...
  it("List all courses - Should return analytical response", async () => {
    const response = await makeASalesRequest("List all available courses");
    ...
    assert.ok(
      ...
      "Should have followUpQuestions array",
    );
  ...
  it("Invalid/unclear question - Should handle gracefully with suggestions", async () => {
    const response = await makeASalesRequest("What is the weather today?");
    ...
    assert.ok(body.error || body.answer, "Should have either error or answer");
  });
