  ...
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    try {
      ...
      if (stepInfo) {
        ...
        console.log(
          `🤖 Generating Cypher query for step ${stepInfo.stepNumber}/${totalSteps}: ${targetQuestion}`,
        );
      } else {
      ...
      if (error) {
        return {
          error: `Failed to generate query: ${error ?? "Unknow error"}`,
        };
      }

      console.log(`✅ Generate Cypher query: ${data?.query}`);
