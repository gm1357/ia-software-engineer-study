...
export const QueryAnalysisSchema = z.object({
  complexity: z
    .enum(["simple", "complex"])
    .describe("Whether the query is simple or complex"),
  requiresDecomposition: z
    .boolean()
    .describe("Whether the query needs to be broken down into sub-queries"),
  subQuestions: z
    .array(z.string())
    .describe(
      "Sub-questions if decomposition is required (empty array if simple)",
    ),
